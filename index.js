...
require("child_process").exec("npm install", async function (err, stdout) {
  ...
  bot.on("ready", async () => {
    ...
    console.log('\x1b[36m%s\x1b[0m', `Thank you for purchasing Ultimate Bot!`)
    console.log('\x1b[36m%s\x1b[0m', `Have an issue? Join our Discord!`)
    console.log(`\x1b[36m%s\x1b[0m`, `discord.lebryantcraft.net`)
    console.log('\x1b[36m%s\x1b[0m', `Bot Activity: ${gFile.type ? gFile.type : "Moderating Server"} ${gFile.activity ? gFile.activity : ""}`)
    console.log(`\x1b[33m`, `#-------------------------------#`);
